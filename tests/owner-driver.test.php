...
$data += ['safetyChecks' => ['cleanEntry', 'seatBelts'], 'cprFirstAid' => 'Current', 'defensiveDriving' => 'Expired', 'passengerAssistance' => 'Never completed', 'confidentialityTraining' => 'Current', 'otherCertifications' => 'Example certification'];
$state = tempnam(__DIR__, 'driver-test-');
$calls = 0;
$sender = function ($received) use (&$calls, $data): void {
    $calls++;
    checkDriver(!isset($received['ssn']) && !isset($received['officeStatus']) && !isset($received['driverActivated']), 'Extra or office data forwarded');
...
try {
    ...
    checkDriver($run(array_replace($data, ['ssn' => 'must-not-forward', 'officeStatus' => 'Active', 'driverActivated' => true])) === 200, 'Valid application rejected');
