...
`/owner-drivers` is linked from the main navigation and footer and included in generated page metadata and the sitemap. The form posts to `/api/owner-driver`, which uses the same private Zoho configuration and PHPMailer dependency as booking notifications. Deploy the updated `dist/` contents (including `.htaccess` and both PHP endpoints), and copy `server/booking.php`, `server/owner-driver.php`, `server/driver-options.json`, `server/driver-program.json`, and `server/driver-internal.txt` into `malydia-private`. Preserve the existing private `config.php` and `vendor/` folder.

Applications go to info@malydiahealth.com with a distinct owner-driver subject. **The waiting list is managed manually in the company mailbox**, not in a dashboard or separate database. Staff should review applications, retain them for consideration, and honor update/removal requests. The form follows the owner-provided application, including legal name/address, license number and expiration, age/licensing/checks/training responses, multi-select availability, experience, vehicle VIN/plate/mileage, seating including driver, and registration/inspection/insurance responses. These details are emailed to the company mailbox; they are not stored in local state files, browser storage, or URLs. The form collects explicit consent for waiting-list contact and retention and does not request identity document copies or financial details. Staff review all responses; selecting No does not automatically reject a preliminary application. Section 4 records checked and unconfirmed vehicle safety items; unchecked items do not block a preliminary application. Section 5 records each training status and optional certifications. Section 6 is an uncompleted staff checklist in the notification email, including all seven status options. Applicant-supplied office fields are discarded; approvals and activation must be handled by staff. This is a manual office record, not an interactive admin dashboard. No approval, position, earnings, or response deadline is promised.
...
Sections 7–9: the proposed model and applicant acknowledgment are public, with required acknowledgment, typed signature, printed name, and date. Classification-review guidance and compensation planning are kept in `server/driver-internal.txt`, outside `public_html`, and appended only to the company notification. They must never be copied to `public/` or imported by frontend code. This planning is not a pay offer. The internal review fields remain blank for staff completion; applicant-provided reviewer or pay fields are discarded. These typed details are self-reported, not identity-verified signatures. The actual planning file is gitignored because this repository is public; GitHub includes only `driver-internal.example.txt`, a blank fallback. Upload all files in the private update ZIP together; retain private SMTP configuration and dependencies.
