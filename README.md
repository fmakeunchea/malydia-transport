...
Applications go to info@malydiahealth.com with a distinct owner-driver subject. **The waiting list is managed manually in the company mailbox**, not in a dashboard or separate database. Staff should review applications, retain them for consideration, and honor update/removal requests. The form follows the owner-provided three-section application, including legal name/address, license number and expiration, age/licensing/checks/training responses, multi-select availability, experience, vehicle VIN/plate/mileage, seating including driver, and registration/inspection/insurance responses. These details are emailed to the company mailbox; they are not stored in local state files, browser storage, or URLs. The form collects explicit consent for waiting-list contact and retention and does not request identity document copies or financial details. Staff review all responses; selecting No does not automatically reject a preliminary application. No approval, position, earnings, or response deadline is promised.
